...
router.get('/vapes', (req, res) => {
    ...
    client.connect()
    .then(() => {
        return client.query('select v.*, s.sabor, m.modelo, mc.marca from vape v inner join sabor s on v.fkid_sabor = s.id_sabor inner join modelo m on v.fkid_modelo = m.id_modelo inner join marca mc on m.fkid_marca = mc.id_marca;');
    })
    .then(result => {
        res.status(200).send(result.rows);
    })
